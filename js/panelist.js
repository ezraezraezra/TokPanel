...
var load_js = function artist_load_js(){
	...
	function connectionDestroyedHandler(event){
		for (var i = 0; i < event.connections.length; i++) {
			...
			if (event.connections[i].connectionId == gaga_id[0]) {
				gaga_id[0] = gaga_id[1];
				gaga_id.splice(1, 2);
				$("#left_speaker_1").attr("id", "left_speaker_temp");
				$("#left_speaker_0").attr("id", "left_speaker_1");
				$("#left_speaker_temp").attr("id", "left_speaker_0");
			}
	...
	function add_to_right_speaker(stream){
