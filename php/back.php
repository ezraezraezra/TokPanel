...
// Get white_house connection id
else if($_GET['comm'] == 'gaga') {
	$arr = getwhite_house($db_selected, $connection, $_GET['view']);
}
// Clean the database
...
function cleanDatabase($db_selected, $connection) {
	...
	$session_update = "UPDATE white_house SET user_status='5' WHERE user_status='2' OR user_status='3' OR user_status='4'";
...
function updateUser($db_selected, $connection, $tb_id, $user_status) {
	...
	# A)
	$session_update = "UPDATE white_house SET user_status='$user_status' WHERE tb_id='$tb_id'";
	$session_update = submit_info($session_update, $connection, true);
...
function promote() {
	...
	# A)
	$session_request = "SELECT * FROM white_house WHERE user_status = '2' ORDER BY unique_id ASC LIMIT 1";
	...
	$session_update = "UDATE white_house SET user_status='3' WHERE unique_id='$on_stage'";
	$session_update = submit_info($session_update, $connection, true);
...
function getwhite_house($db_selected, $connection, $view_num) {
	//$on_stage;
	//echo "This is pre on_stage: ".$on_stage;
	$session_request = "SELECT * FROM white_house WHERE user_status = '4'";
	...
	if(isset($on_stage) == true) {
		if($view_num != 5) {
			$arr = array("status"=>'200', "gaga_id"=>$on_stage[$view_num]);
		}
		else {
...
function getState($db_selected, $connection) {	
	...
	# A)
	$stage_request = "SELECT * FROM white_house WHERE user_status = '3'";
	...
	# B)
	$queue_request = "SELECT * FROM white_house WHERE user_status = '2'";
...
function addUser($db_selected, $connection, $tb_id, $user_status) {
	...
	# A)
	$session_request = "INSERT INTO white_house (tb_id, user_status) VALUES('$tb_id', '$user_status')";
	$session_request = submit_info($session_request, $connection, true);
...
function removeUser($db_selected, $connection, $tb_id, $user_status) {
	...
	# A)
	$session_request = "UPDATE white_house SET user_status='$user_status' WHERE tb_id='$tb_id'";
	$session_request = submit_info($session_request, $connection, true);
